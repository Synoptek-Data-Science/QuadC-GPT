...
def save_docs_to_vector_db(
    ...
    # Start overall process timing
    overall_start_time = time.time()
    
    log.info(f"Starting save_docs_to_vector_db with {len(docs)} documents")
    
    def _get_docs_info(docs: list[Document]) -> str:
        docs_info = set()
        for doc in docs:
            metadata = getattr(doc, "metadata", {})
            doc_name = metadata.get("name", "") or metadata.get("title", "") or metadata.get("source", "")
            if doc_name:
    ...
    if metadata and "hash" in metadata:
        try:
            ...
            if "DUPLICATE_CONTENT" in str(e):
                raise
            log.warning(f"Error checking for duplicates: {e}")
    duplicate_check_time = time.time() - duplicate_check_start_time
    log.info(f"Duplicate check completed in {duplicate_check_time:.2f}s")
    
    # Document splitting
    log.info(f"================document splitting===================")
    if split:
        doc_splitting_start_time = time.time()
        log.info(f"Starting document splitting...")

        # >>> INSERT DYNAMIC ADJUSTMENT CODE HERE <
        original_chunk_size = request.app.state.config.CHUNK_SIZE
        original_chunk_overlap = request.app.state.config.CHUNK_OVERLAP
        total_content_size = sum(len(doc.page_content) for doc in docs)
        log.info(f"Total content size: {total_content_size} characters")
        print("TOTAL_CONTENT SIZE: ", total_content_size)
        BIG_CONTENT_THRESHOLD = 400000 
        
        if total_content_size > BIG_CONTENT_THRESHOLD:
            adjusted_chunk_size = original_chunk_size * 2
            adjusted_chunk_overlap = original_chunk_overlap * 2
            log.info(f"Big content detected. Adjusting chunk_size from {original_chunk_size} to {adjusted_chunk_size}")
            log.info(f"Adjusting chunk_overlap from {original_chunk_overlap} to {adjusted_chunk_overlap}")
            request.app.state.config.CHUNK_SIZE = adjusted_chunk_size
            request.app.state.config.CHUNK_OVERLAP = adjusted_chunk_overlap
        else:
            log.info(f"Normal content size. Using default chunk_size={original_chunk_size}, chunk_overlap={original_chunk_overlap}")
        ...
        try:
            if request.app.state.config.TEXT_SPLITTER in ["", "character"]:
                ...
                for i in range(0, len(docs), batch_size):
                    ...
                    total_split_time += batch_time
                    log.info(f"Split batch {i//batch_size + 1}/{(len(docs) + batch_size - 1)//batch_size} in {batch_time:.2f}s")
                ...
                for i in range(0, len(docs), batch_size):
                    ...
                    for doc in batch:
                        ...
                        for split_chunk in md_header_splits:
                            headings_list = [split_chunk.metadata.get(header_name, "") 
                                           for _, header_name in headers_to_split_on 
                                           if header_name in split_chunk.metadata]
                ...
                raise ValueError(f"Invalid text splitter: {request.app.state.config.TEXT_SPLITTER}")
            
            docs = split_docs
            doc_splitting_time = time.time() - doc_splitting_start_time
            log.info(f"Document splitting completed in {doc_splitting_time:.2f}s (Total batch time: {total_split_time:.2f}s)")
            
             # >>> INSERT RESTORATION CODE HERE <
            if total_content_size > BIG_CONTENT_THRESHOLD:
                request.app.state.config.CHUNK_SIZE = original_chunk_size
                request.app.state.config.CHUNK_OVERLAP = original_chunk_overlap
                log.info(f"Restored original chunk_size={original_chunk_size}, chunk_overlap={original_chunk_overlap}")
                
        except Exception as e:
            log.error(f"Error during document splitting: {str(e)}")
    ...
    log.info(f"================prepare text and metadata===================")
    ...
    log.info(f"Text and metadata preparation completed in {preparation_time:.2f}s")

    # Collection management
    log.info(f"================db collection management===================")
    try:
        ...
        if VECTOR_DB_CLIENT.has_collection(collection_name=collection_name):
            ...
            if overwrite:
                ...
                log.info(f"Collection exists, overwrite=False, add=False - returning")
                return True
        collection_check_time = time.time() - collection_check_start_time
        log.info(f"Collection management completed in {collection_check_time:.2f}s")
    except Exception as e:
        log.error(f"Error checking/managing collection: {str(e)}")
        raise Exception(f"Collection management failed: {str(e)}")
    
    # Generate embeddings
    log.info(f"================generate embedding===================")
    try:
        ...
        embedding_function = get_embedding_function(
            ...
            request.app.state.config.RAG_EMBEDDING_MODEL,
            request.app.state.ef,
            (request.app.state.config.RAG_OPENAI_API_BASE_URL if request.app.state.config.RAG_EMBEDDING_ENGINE == "openai"
             else (request.app.state.config.RAG_OLLAMA_BASE_URL if request.app.state.config.RAG_EMBEDDING_ENGINE == "ollama"
                   else request.app.state.config.RAG_AZURE_OPENAI_BASE_URL)),
            (request.app.state.config.RAG_OPENAI_API_KEY if request.app.state.config.RAG_EMBEDDING_ENGINE == "openai"
             else (request.app.state.config.RAG_OLLAMA_API_KEY if request.app.state.config.RAG_EMBEDDING_ENGINE == "ollama"
                   else request.app.state.config.RAG_AZURE_OPENAI_API_KEY)),
            request.app.state.config.RAG_EMBEDDING_BATCH_SIZE,
            azure_api_version=(request.app.state.config.RAG_AZURE_OPENAI_API_VERSION 
                             if request.app.state.config.RAG_EMBEDDING_ENGINE == "azure_openai" else None),
        )
        
        # Batch embedding generation
        log.info(f"================batch embedding generation===================")
        EMBEDDING_BATCH_SIZE = 100
        all_embeddings = []
        total_embedding_batch_time = 0
        ...
        for i in range(0, len(texts), EMBEDDING_BATCH_SIZE):
            try:
                ...
                batch_embedding_time = time.time() - batch_embedding_start_time
                total_embedding_batch_time += batch_embedding_time
                log.info(f"Generated embeddings for batch {i//EMBEDDING_BATCH_SIZE + 1}/{(len(texts) + EMBEDDING_BATCH_SIZE - 1)//EMBEDDING_BATCH_SIZE} in {batch_embedding_time:.2f}s")
            except Exception as e:
                log.error(f"Error generating embeddings for batch {i//EMBEDDING_BATCH_SIZE + 1}: {str(e)}")
                raise Exception(f"Embedding generation failed at batch {i//EMBEDDING_BATCH_SIZE + 1}: {str(e)}")
        
        embeddings = all_embeddings
        total_embedding_time = time.time() - embedding_start_time
        log.info(f"Generated {len(embeddings)} embeddings in {total_embedding_time:.2f}s (Total batch time: {total_embedding_batch_time:.2f}s)")
        
    except Exception as e:
        log.error(f"Embedding generation failed: {str(e)}")
        raise Exception(f"Embedding generation failed: {str(e)}")
    
    # Insert into vector DB
    log.info(f"================insert data into vector db===================")
    try:
        items = [
            {
                "id": str(uuid.uuid4()),
        ...
        MAX_RETRIES = 2
        total_insertion_batch_time = 0        
        
        for i in range(0, len(items), VECTOR_INSERT_BATCH_SIZE):
            batch_items = items[i:i + VECTOR_INSERT_BATCH_SIZE]
            batch_num = i//VECTOR_INSERT_BATCH_SIZE + 1
            total_batches = (len(items) + VECTOR_INSERT_BATCH_SIZE - 1)//VECTOR_INSERT_BATCH_SIZE
            
            for retry in range(MAX_RETRIES):
                try:
                    batch_insertion_start_time = time.time()
                    log.info(f"Inserting batch {batch_num}/{total_batches} (attempt {retry + 1}/{MAX_RETRIES})")
                    VECTOR_DB_CLIENT.insert(
                        collection_name=collection_name,
                        items=batch_items,
                    )
                    batch_insertion_time = time.time() - batch_insertion_start_time
                    total_insertion_batch_time += batch_insertion_time
                    log.info(f"Successfully inserted batch {batch_num}/{total_batches} in {batch_insertion_time:.2f}s")
                    break
                except Exception as e:
                    log.warning(f"Error inserting batch {batch_num} (attempt {retry + 1}): {str(e)}")
                    if retry == MAX_RETRIES - 1:
                        raise Exception(f"Failed to insert batch {batch_num} after {MAX_RETRIES} attempts: {str(e)}")
                    time.sleep(2)
        
        total_insertion_time = time.time() - data_insertion_start_time
        log.info(f"Inserted {len(items)} items in {total_insertion_time:.2f}s (Total batch time: {total_insertion_batch_time:.2f}s)")
        
        # Log overall process time
        overall_time = time.time() - overall_start_time
        log.info(f"=== COMPLETE PROCESS SUMMARY ===")
        log.info(f"  - Duplicate check: {duplicate_check_time:.2f}s")
        if split:
            log.info(f"  - Document splitting: {doc_splitting_time:.2f}s")
        log.info(f"  - Text preparation: {preparation_time:.2f}s")
        log.info(f"  - Collection management: {collection_check_time:.2f}s")
        log.info(f"  - Embedding generation: {total_embedding_time:.2f}s")
        log.info(f"  - Vector DB insertion: {total_insertion_time:.2f}s")
        log.info(f"Total process time (file upload to vector DB insertion): {overall_time:.2f}s")
        log.info(f"================================")
        
        return True
        
    except Exception as e:
        log.error(f"Vector DB insertion failed: {str(e)}")
        raise Exception(f"Vector DB insertion failed: {str(e)}")
        ...
        try:
            ...
            if form_data.content:
                ...
                if file_path:
                    ...
                    # Check if file is CSV or Excel to bypass Tika processing
                    file_extension = file.filename.lower().split('.')[-1]
                    bypass_tika_extensions = ['csv', 'xls', 'xlsx', 'xlsm', 'xlsb']
